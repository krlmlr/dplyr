...
#' @export
group_keys_data_frame <- function(.tbl, ...) {
  if (dots_n(...) > 0) {
...
#' @export
group_indices_data_frame <- function(.data, ...) {
  if (dots_n(...) > 0) {
    lifecycle::deprecate_warn(
      "1.0.0", "group_indices(... = )",
      details = "Please `group_by()` first",
...
#' @export
group_size_data_frame <- function(x) {
...
#' @export
n_groups_data_frame <- function(x) {
  nrow(group_data(x))
}
