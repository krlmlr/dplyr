...
test_that("rows_update() allows `y` keys that don't exist in `x` to be ignored", {
  ...
  expect_identical(
    rows_update(x, y, "a", nonexistent = "ignore"),
    tibble(a = 1, b = 1)
  )
})
...
test_that("`nonexistent` is validated", {
  ...
  expect_snapshot({
    (expect_error(rows_update(x, y, by = "a", nonexistent = "foo")))
    (expect_error(rows_update(x, y, by = "a", nonexistent = 1)))
  })
})
...
test_that("rows_patch() allows `y` keys that don't exist in `x` to be ignored", {
  ...
  expect_identical(
    rows_patch(x, y, "a", nonexistent = "ignore"),
    tibble(a = 1, b = 1)
  )
})
...
test_that("rows_delete() allows `y` keys that don't exist in `x` to be ignored", {
  ...
  expect_identical(
    rows_delete(x, y, "a", nonexistent = "ignore"),
