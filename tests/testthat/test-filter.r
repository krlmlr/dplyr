...
test_that("filter propagates attributes", {
  ...
  test2 <- test %>%
    filter(Date < ISOdate(2010, 01, 01, 5))
...
test_that("date class remains on filter (#273)", {
  ...
  x1.filter <- x1 %>%
    filter(as.Date(date) > as.Date("2013-01-01"))
  x2$date <- x2$date + 1
  x2.filter <- x2 %>%
    filter(as.Date(date) > as.Date("2013-01-01"))
...
test_that("`vars` attribute is not added if empty (#2772)", {
  expect_identical(tibble(x = 1:2) %>%
    filter(x == 1), tibble(x = 1L))
})
