...
test_that("unnamed elements must return data frames", {
  expect_error(
    df %>%
      ungroup() %>%
      do(1), "Result must be a data frame, not numeric"
  )
  expect_error(
    df %>%
      do(1), "Results 1, 2, 3 must be data frames, not numeric"
  )
  expect_error(
    df %>%
      do("a"), "Results 1, 2, 3 must be data frames, not character"
  )
})
...
test_that("empty inputs give empty outputs (#597)", {
  out <- data.frame(a = numeric(), b = factor()) %>%
    group_by(b) %>%
    do(data.frame())
  expect_equal(out, data.frame(b = factor()) %>%
    group_by(b))

  out <- data.frame(a = numeric(), b = character()) %>%
    group_by(b) %>%
    do(data.frame())
  expect_equal(out, data.frame(b = character()) %>%
    group_by(b))
})
