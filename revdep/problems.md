...
*   checking tests ... ERROR
    ...
    Complete output:
      ...
      `actual$default_duckdb_connection$con@conn_ref` is <pointer: 0x559e5707cb30>
      `expected$default_duckdb_connection$con@conn_ref` is <pointer: 0x559e5b77fe20>
      
      `actual$default_duckdb_connection$con@driver@database_ref` is <pointer: 0x559e5707d3d0>
      `expected$default_duckdb_connection$con@driver@database_ref` is <pointer: 0x559e5b779960>
      ...
      `names(expected$duckplyr_macros)[1:4]`: "___null" "<" "<=" ">"
           ...
           names(actual$duckplyr_macros) | names(expected$duckplyr_macros)     
      [10] "___log10"                    | "___log10"                      [11]
      [11] "___log"                      | "___log"                        [12]
      [12] "grepl"                       | "grepl"                         [13]
      [13] "as.integer"                  -                                     
      [14] "if_else"                     | "if_else"                       [14]
      [15] "|"                           | "|"                             [15]
      [16] "&"                           | "&"                             [16]
      
      actual$duckplyr_macros[1:16] vs expected$duckplyr_macros[1:16]
      - <"(x, y) AS x < y"
      ...
      body(actual$rel_translate)[8:19] vs body(expected$rel_translate)[8:20]
        `        env <- quo_get_env(quo)`
        `    }`
        `    used <- character()`
      ...
      + `            logical = if (top_level && length(expr) == 1 && is.na(expr)) relexpr_function("___null", `
      + `                list()) else relexpr_constant(expr), symbol = {`
      and 3 more ...
      
      body(actual$rel_translate)[35:40] vs body(expected$rel_translate)[36:48]
        `                  name <- name[[3]]`
        `                } else {`
        `                  pkg <- NULL`
        `                }`
      ...
      body(actual$rel_translate)[60:75] vs body(expected$rel_translate)[68:98]
      ...
      body(actual$rel_translate)[85:91] vs body(expected$rel_translate)[108:115]
        `                })`
        `                aliases <- c(sd = "stddev", first = "first_value", `
        `                  last = "last_value", nth = "nth_value", \`/\` = "___divide", `
      - `                  log10 = "___log10", log = "___log", NULL)`
      + `                  log10 = "___log10", log = "___log", as.integer = "r_base::as.integer", `
      + `                  NULL)`
        `                known_window <- c("rank", "rank_dense", "dense_rank", `
        `                  "percent_rank", "row_number", "first_value", `
        `                  "last_value", "nth_value", "cume_dist", "lead", `
      
      body(actual$rel_translate)[101:106] vs body(expected$rel_translate)[125:133]
      ...
      body(actual$rel_translate)[124:139] vs body(expected$rel_translate)[151:176]
        `                  }`
        `                }`
